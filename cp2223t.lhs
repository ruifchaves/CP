...
\end{spec}
Sabemos que |f a b c (n + 2) = f1 a b c n|, então
\begin{spec}
...
\end{code}
Obtemos então 3 funções mutuamente recursivas, sendo a função |fnew| a que guarda o resultado.
Aplicando então a regra prática do anexo B, chegamos de forma trivial à definição do |loop| e do |init| para chegar à definição de |fbl| (função matematicamente igual a |f|, mas mais eficiente)
\begin{code}
loop a b c ((f2,f1), fnew) = ((f1,a*f1+b*f2+c*fnew),f2)
initial = ((1,1),0)
wrap = p2
\end{code}
De seguida encontra-se a tabela \ref{table:tempos} com os testes de performance, comparando os tempos de execução da função |f| e da função |fbl| (valores dos argumentos |a b c| não influênciam os tempos de execução, logo a tabela pode ser considerada genérica para qualquer valor que estes possam tomar)
\begin{table}[ht!]
\centering\small
\begin{center}
\begin{tabular}{||l||l||l||l||}
\hline
valor de |n| & tempo de execução de |f| (s) & tempo de execução de |fbl| (s) & tempo |f| / tempo |fbl| 
\\\hline
5 & 0.01 & 0.01 & 1 
\\\hline
10 & 0.01 & 0.01 & 1
...
\end{code}
Inicialmente, para facilitar, começamos por criar uma função que recebe uma Carpete (Lista de |Squares|) e apresenta a imagem num ficheiro html (A função |present1Carpet|). Para isso basta recorrer à função |drawSq| utilizando como argumento o quadrado central base ((0,0),32), mas falta ainda saber a profundidade a que corrresponde essa Carpete. Foi então necessário criar outra função auxiliar que em função do nº de quadrados na lista vai indicar a profundidade respetiva (a função |seqFormula|, que é simplesmente uma fórmula matemática para esta sequência), então passamos mais o argumento |seqFormula| da |length| da lista à função |drawSq|, obtendo a profundidade respetiva. Assim a função |present1Carpet| já apresenta o ficheiro html recebendo apenas auma lista de quadrados.

Para apresentar as imagens de todas as Carpetes até uma certa profundidade basta primeiro correr a função |carpets|, para se obter a lista de Carpetes e utilizar essa lista como argumento na função |present|, que vai aplicar o monadic map |presente1Carpet| a todas as carpetes (mas só depois de fazer reverse à lista, para os ficheiros surgirem por ordem crescente de profundidade) 
\begin{code}
...
A função |consolidade| é um catamorfismo, que recebe uma lista de pares |Team >< Int|, sendo que cada par especifica os pontos que a respetiva equipa obteve num jogo, como a lista reflete todos os pontos de todas os jogos de todas as equipas de um grupo, vai obrigatoriamente ter equipas repetidas, com outros pontos associados a outros jogos. O objetivo da função é devolver a lista já com os pontos todos que a equipa obteve acumulados num só par, para todas as equipas, sendo agora uma lista de 4 elementos. 

Inicialmente foi feita uma função auxiliar |consolAux|, que recebendo um par | Team >< Int | e a lista de todos os pares |Team >< Int|, vai juntar a sua pontuação com a do primeiro elemento da lista com a mesma equipa e mantém o resto da lista inalterada.

Agora para o gene em si, repara-se que será um either de 1 ou |(Team >< Int)| |><| |(Team >< Int)*|. No lado esquerdo apenas se retorna a lista vazia, e no direito caso a lista seja vazia, apenas se faz singl ao par, para o transformar numa lista, se a lista não for vazia basta chamar a função auxiliar definida para agrupar os pontos da equipa na lista com todos os pontos de todas as equipas, uma vez que podemos pensar que a parte recursiva da lista já foi feita,estando já os pontos agrupados pelas equipas.
